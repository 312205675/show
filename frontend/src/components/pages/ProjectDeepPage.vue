<template>
  <div class="page project-deep-page">
    ...
    <!-- Health Dashboard -->
    <div class="health-dashboard" :class="severity">
    ...
    <div class="visual-grid">
          ...
          <div v-for="(s, i) in data.funnel" :key="i" class="funnel-stage"
            ...
            <div class="funnel-bar-wrap">
              <div class="funnel-bar" :style="funnelBarStyle(s, i)">
                <div class="funnel-bar-fill-inner" />
                <span class="funnel-name">{{ s.name }}</span>
                <span class="funnel-count">{{ s.count.toLocaleString() }}</span>
              </div>
            </div>
            <div v-if="i > 0" class="funnel-conversion" :class="{ 'conv-danger': (s.count / data.funnel[i - 1].count) < 0.4 }">
              <div class="conversion-arrow" />
              <span class="conversion-text">{{ ((s.count / data.funnel[i - 1].count) * 100).toFixed(0) }}%</span>
            </div>
          </div>
        </div>
        <!-- Loss analysis -->
        <div class="funnel-loss-row">
          <div v-for="(s, i) in data.funnel" :key="'l' + i" class="loss-chip" v-show="i > 0"
            ...
            <span class="loss-pct">{{ ((1 - s.count / data.funnel[i - 1].count) * 100).toFixed(0) }}%流失</span>
          </div>
        </div>
      </div>

      <!-- Customer Sources - Donut -->
      ...
      <div class="card segment-card">
        ...
        <div class="segment-visual">
          <div v-for="seg in data.customerSegments" :key="seg.segment" class="segment-row">
            <div class="seg-avatar" :style="{ background: seg.color + '20', color: seg.color }">
            ...
            <div class="seg-body">
              ...
              <div class="seg-bar-track">
                <div class="seg-bar-fill" :style="{ width: seg.percent + '%', background: `linear-gradient(90deg, ${seg.color}66, ${seg.color})` }" />
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Inventory Structure - Risk Heatmap -->
...
// Ring style computed to avoid CSS custom property issues
const ringStyle = computed(() => {
  const score = data.value.aiDiagnosis.score
  const color = scoreColor.value
  return {
    background: `conic-gradient(${color} ${score}%, rgba(255, 255, 255, 0.05) ${score}%)`,
    boxShadow: severity.value === 'danger' ? `0 0 24px rgba(239, 68, 68, 0.3)` : severity.value === 'warn' ? `0 0 20px rgba(245, 158, 11, 0.2)` : `0 0 20px rgba(0, 0, 0, 0.3)`,
  }
})

const overallConversion = computed(() => {
  if (data.value.funnel.length < 2) return 0
...
// Executive alerts - what a boss needs to see immediately
...
// Financial KPIs - executive focus
...
function funnelBarStyle(s: { count: number; color: string }, i: number) {
...
.score-unit { font-size: 12px; color: $text-caption; font-weight: 500; margin-top: 4px; }

.health-label-wrap { text-align: center; }
.health-status {
  ...
  &.danger { color: $danger; background: rgba(239, 68, 68, 0.15); animation: blink 1.5s ease-in-out infinite; }
}

.health-sublabel { display: block; font-size: 10px; color: $text-caption; margin-top: 3px; }
...
.problem-detail { display: flex; flex-direction: column; gap: 3px; }
...
@media (max-width: 768px) {
  ...
  .health-left { width: auto; flex-direction: row; justify-content: center; }
}
</style>
